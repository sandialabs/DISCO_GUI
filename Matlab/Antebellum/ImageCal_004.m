...
Version Release Notes
    Version ImageCal_004 employs a different strategy in the  Re-mask B!! 
section. It first attempts to find the bounding box specified by Radius,
and the individual Pupil's Center (h,k). It will then create a mask that
fits the circle equation using Radius.
...
p_mask = mask(raw2,centers(pupil_num,:),radii(pupil_num)); %pupil mask
...
% Prep the destination by removing the pupil 

%  sub_im = 0.*Pupil.Im;

% subpts = calcEndPts(pupil_dest,centers,max_r);
subpts = calcEndPts(pupil_dest,centers,radii(pupil_num));
